...
test("plugin tasks coalesce, share schedules, cancel queued and active work, and report uncertain outcomes",async()=>{
  ...
  const firstRun=controlledTask({active:0,peak:0},1,gates);const first=new PluginTaskManager("first","PUBLIC",queue,(event,details)=>events.push({event,details}));first.register("refresh",(_,task)=>firstRun(task.signal));
  ...
  const second=new PluginTaskManager("second","PUBLIC",queue,(event,details)=>events.push({event,details}));second.register("refresh",async()=>"never");const queued=second.enqueue("refresh");await tick();const secondStopped=second.stop();await assert.rejects(queued,(error)=>error.code==="TASK_CANCELLED"&&error.outcome==="not-started");await secondStopped;
  const firstStopped=first.stop();await assert.rejects(blocker,(error)=>error.code==="TASK_CANCELLED"&&error.outcome==="uncertain");await firstStopped;assert(events.some(({event,details})=>event==="task-cancelled"&&details.outcome==="uncertain"));
  const scheduler=new SharedTaskQueue();const scheduledEvents=[];let calls=0;const scheduled=new PluginTaskManager("scheduled","PUBLIC",scheduler,(event)=>scheduledEvents.push(event));scheduled.register("refresh",async()=>{calls+=1});scheduled.schedule("refresh",{intervalMs:100,runImmediately:true,coalesceKey:"collection"});await new Promise((resolve)=>setTimeout(resolve,130));await scheduled.stop();assert(calls>=1);assert(scheduledEvents.includes("task-queued"));
});
...
test("Product Hunt INTERCEPT collection validates, persists, and serializes with COOKIE work",async()=>{
  const productionAdapter=await readFile(path.join(root,"resources/opencli/node_modules/@jackwener/opencli/clis/producthunt/hot.js"),"utf8");assert.match(productionAdapter,/strategy:\s*Strategy\.INTERCEPT/);assert.match(productionAdapter,/installInterceptor\('producthunt\.com'\)/);
  ...
  try {
    ...
    const calls=(await readFile(`${stateFile}.calls`,"utf8")).trim().split(/\r?\n/).map(JSON.parse);let browserActive=0;let browserPeak=0;for(const call of calls.filter(({command})=>command.startsWith("producthunt")||command.startsWith("zhihu"))){browserActive+=call.type==="start"?1:-1;browserPeak=Math.max(browserPeak,browserActive)}assert.equal(browserPeak,1);
    await api(runtime.message.origin,"product-hunt",`read?url=${encodeURIComponent(productHunt.products[0].url)}`,"POST");await stopRuntime(runtime.child);const store=openStore(path.join(dataRoot,"product-hunt","product-hunt.sqlite"));assert.equal(store.schemaVersion(),1);assert.equal(store.snapshotCount(),1);assert.equal(store.list()[0].read,true);store.close();
