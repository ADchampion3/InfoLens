...
test("plugin-owned SQLite migrations and validators reject malformed public results", async () => {
  ...
  try {
    ...
    assert.equal(hn.schemaVersion(),1); assert.equal(gh.schemaVersion(),1); assert.deepEqual({...hn.settings()},{policy:"manual",intervalMinutes:60}); assert.deepEqual({...gh.settings()},{policy:"manual",intervalMinutes:60});
    hn.close(); gh.close();
    const reopened=openHnStore(path.join(temp,"hn.sqlite")); assert.equal(reopened.schemaVersion(),1); reopened.close();
...
test("both PUBLIC plugins persist independently, reopen retained content, and preserve it after failures", async () => {
  ...
  try {
    ...
    assert.deepEqual(await api(origin,"hn","settings?policy=fixed&intervalMinutes=360","POST"),{policy:"fixed",intervalMinutes:360}); assert.deepEqual(await api(origin,"github-trending","settings"),{policy:"manual",intervalMinutes:60});
