...
test("Product Hunt INTERCEPT collection validates, persists, and serializes with COOKIE work",async()=>{
  ...
  try {
    runtime=await startRuntime(dataRoot,stateFile);assert.deepEqual(runtime.message.plugins.map(({id})=>id).sort(),["github-trending","hn","juejin","product-hunt","zhihu-hot"]);
