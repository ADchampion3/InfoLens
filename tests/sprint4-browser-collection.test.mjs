...
test("Zhihu schema migration and native-row validation are deterministic",async()=>{
  ...
  try{const store=openStore(path.join(temp,"zhihu.sqlite"));assert.equal(store.schemaVersion(),1);assert.deepEqual({...store.settings()},{policy:"manual",intervalMinutes:60});assert.equal(store.metadata().dependencyState,"unknown");store.close();const reopened=openStore(path.join(temp,"zhihu.sqlite"));assert.equal(reopened.schemaVersion(),1);reopened.close();assert.throws(()=>validateCollection([]),/must contain rows/);assert.throws(()=>validateCollection([{rank:1,title:"bad"}]),/invalid/);assert.equal(validateAuthStatus([{logged_in:true,site:"zhihu"}]),true);assert.throws(()=>validateAuthStatus([{logged_in:false,site:"zhihu"}]),error=>error.code==="SITE_LOGIN_REQUIRED") }finally{await rm(temp,{recursive:true,force:true})}
});
