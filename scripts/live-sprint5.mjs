...
const root=path.resolve(import.meta.dirname,"..");const profile=path.join(root,".infolens-live","sprint5");await mkdir(profile,{recursive:true});
...
const url=`http://127.0.0.1:4176/?runtimeOrigin=${encodeURIComponent(origin)}`;await writeFile(path.join(profile,"url.txt"),url);process.stdout.write(`Live Sprint 5 preview: ${url}\n`);
