...
packages/plugin-distribution/  deterministic ZIP artifacts and distribution sources
...
The repository's fixed `plugins/` directory is the development and daemon discovery location for built packages. Personal Plugin Distribution stages a deterministic ZIP outside that directory, validates it, and commits the extracted package into the same managed location. The MVP does not distinguish official and user-installed package locations; every discovered package has the same daemon-owned lifecycle and removal semantics.
...
Before committing a Distribution Artifact into the managed plugin directory, the daemon bounds and safely inspects the ZIP, then checks that `contractVersion` is supported, that its own semantic version satisfies `minHostVersion`, and that every declared command mapping is supported. The daemon repeats those checks for every package during startup discovery. It rejects an incompatible package with the reason before any installation changes or module activation. A rejected discovered package is not activated or shown in ordinary navigation, but remains visible in plugin management for inspection and removal. The daemon otherwise performs only structural package validation needed for discovery and startup. Installed local plugins are trusted by default: there is no permission approval, package review, data-generation system, or publisher-controlled release workflow.

At startup, the daemon scans the project's fixed `plugins/` directory for plugin packages. Personal installation accepts either a local ZIP or a direct HTTPS ZIP URL with an expected SHA-256, stages it outside the managed directory, and enables it immediately after successful validation. A valid discovered plugin with no prior daemon state is enabled by default; a user-disabled plugin remains disabled. MVP development builds and runs official plugins in their repository directories; it does not support external development links, symbolic links, or plugin hot reload.

An existing external Plugin ID requires an explicit replacement intent; a Bundled Plugin cannot be replaced by Personal Plugin Distribution. Replacement and rollback are serialized per Plugin. Before switching, the daemon snapshots the package, Plugin-owned data, Adapter Scope, Host State metadata, and enabled state, retains exactly one complete previous revision, and restores that revision if deactivation, disk, validation, switching, or activation fails. A rollback swaps the current and previous complete revisions; it does not perform a data migration.

Explicit plugin removal asks the daemon to deactivate the plugin module, cancel its tasks, and unregister its routes before deleting its package, its plugin-owned data directory, its Adapter Scope, and retained Distribution Revisions. If the module does not settle within a short grace period, the daemon restarts its in-process Plugin Runtime without that module before deletion. The daemon does not retain source data beyond the one explicit replacement revision because a newly installed plugin package may use an incompatible data format.
