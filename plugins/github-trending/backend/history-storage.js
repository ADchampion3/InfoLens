...
function createStore(db, filename) {
  ...
  store={
    ...
    snapshotDates(){return db.prepare("SELECT date(collected_at,'localtime') AS date,COUNT(*) AS snapshotCount FROM collection_snapshots GROUP BY date(collected_at,'localtime') ORDER BY date DESC").all()},
