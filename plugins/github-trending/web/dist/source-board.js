...
function repositoryCard(repository, index, readOnly = false) {
  ...
  const gained = document.createElement("span"); gained.textContent = `+${compact(repository.starsGained)} stars`;
  top.append(rank, gained);
  const button = document.createElement("button"); button.className = "card-action"; button.type = "button"; button.dataset.repositoryId = repository.id; button.setAttribute("aria-label", `Read ${repository.id}`);
  ...
  const metrics = document.createElement("span"); metrics.textContent = `${compact(repository.stars)} stars · ${compact(repository.forks)} forks`;
