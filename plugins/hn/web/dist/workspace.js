...
function row(story, readOnly=false) {
  ...
  const title=document.createElement("button"); title.className="story-title"; title.textContent=story.title; title.onclick=async()=>{ open(story.url); if(readOnly)return; data=await request(`read?id=${encodeURIComponent(story.id)}`,{method:"POST"}); render(data); };
