...
function questionButton(question, readOnly) {
  ...
  const meta = document.createElement("small"); meta.textContent = `${question.heat} · ${question.answers} answers`;
...
function renderReader(question, readOnly) {
  ...
  if (!question) return;
  const kicker = document.createElement("p"); kicker.className = "reader-kicker"; kicker.textContent = `${readOnly ? "HISTORY" : "QUESTION"} / #${String(question.rank).padStart(2, "0")}`;
  ...
  const note = document.createElement("p"); note.className = "reader-note"; note.textContent = readOnly ? "当前查看历史快照；打开链接不会修改快照。" : "选择左侧其他问题，Workspace 会保留当前阅读位置。";
