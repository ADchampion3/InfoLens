...
function openLegacyStore(filename) {
  ...
  if (version < 1) db.exec(`
    ...
    CREATE TABLE questions (
      url TEXT PRIMARY KEY, rank INTEGER NOT NULL, title TEXT NOT NULL, excerpt TEXT,
      heat TEXT NOT NULL, answers INTEGER NOT NULL, thumbnail_url TEXT,
  ...
  const upsert = db.prepare(`
    INSERT INTO questions (url,rank,title,excerpt,heat,answers,thumbnail_url,is_read,collected_at)
    VALUES (?,?,?,?,?,?,?,COALESCE((SELECT is_read FROM questions WHERE url=?),0),?)
    ON CONFLICT(url) DO UPDATE SET rank=excluded.rank,title=excluded.title,excerpt=excluded.excerpt,
      heat=excluded.heat,answers=excluded.answers,thumbnail_url=excluded.thumbnail_url,collected_at=excluded.collected_at
  `);
  ...
  return {
    ...
    list() { return db.prepare("SELECT url,rank,title,excerpt,heat,answers,thumbnail_url AS thumbnailUrl,is_read AS read FROM questions ORDER BY rank").all().map((row) => ({ ...row, read: Boolean(row.read) })); },
    ...
    replace(questions, collectedAt) {
      db.exec("BEGIN IMMEDIATE");
      try {
        for (const question of questions) upsert.run(question.url,question.rank,question.title,question.excerpt,question.heat,question.answers,question.thumbnailUrl,question.url,collectedAt);
        const urls = new Set(questions.map(({url}) => url));
        for (const {url} of db.prepare("SELECT url FROM questions").all()) if (!urls.has(url)) db.prepare("DELETE FROM questions WHERE url=?").run(url);
        db.prepare("INSERT INTO collection_snapshots(collected_at,record_count,payload) VALUES (?,?,?)").run(collectedAt,questions.length,JSON.stringify(questions));
    ...
    markRead(url, read) { db.prepare("UPDATE questions SET is_read=? WHERE url=?").run(read?1:0,url); },
