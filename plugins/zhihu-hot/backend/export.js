...
function record(value) {
  return {
    ...
    excerpt: value?.excerpt === null || value?.excerpt === undefined ? "" : String(value.excerpt),
    heat: String(value?.heat ?? ""),
    answers: value?.answers ?? "",
    thumbnailUrl: value?.thumbnailUrl ? String(value.thumbnailUrl) : "",
  };
}
...
function stateMap(reader) {
  return new Map(reader.prepare("SELECT question_url AS id,is_read AS read FROM question_user_state ORDER BY question_url").all().map(({ id, read }) => [identity(id), Boolean(read)]));
}
...
function csvExport(reader, pluginId, pluginVersion, exportedAt) {
  return (function* () {
    try {
      ...
      yield csvRow(["snapshot_collected_at", "url", "rank", "title", "excerpt", "heat", "answers", "thumbnail_url", "read"]);
      const states = stateMap(reader);
      for (const row of reader.prepare("SELECT collected_at AS collectedAt,payload FROM collection_snapshots ORDER BY collected_at,id").iterate()) {
        for (const item of parseSnapshot(row).map(record)) yield csvRow([row.collectedAt, item.url, item.rank, item.title, item.excerpt, item.heat, item.answers, item.thumbnailUrl, states.get(item.url) ?? false]);
      }
      yield "\n";
...
function markdownExport(reader, pluginId, pluginVersion, exportedAt) {
  return (function* () {
    try {
      yield `# Zhihu Hot history\n\n- Plugin: ${markdown(pluginId)}\n- Version: ${markdown(pluginVersion)}\n- Exported: ${markdown(exportedAt)}\n`;
      const states = stateMap(reader);
      for (const row of reader.prepare("SELECT collected_at AS collectedAt,payload FROM collection_snapshots ORDER BY collected_at,id").iterate()) {
        yield `\n## Snapshot ${markdown(row.collectedAt)}\n\n| Rank | Question | Excerpt | Heat | Answers | Read |\n| ---: | --- | --- | ---: | ---: | :---: |\n`;
        for (const item of parseSnapshot(row).map(record)) yield `| ${item.rank} | [${markdown(item.title)}](${item.url}) | ${markdown(item.excerpt)} | ${markdown(item.heat)} | ${item.answers} | ${states.get(item.url) ?? false} |\n`;
      }
      yield "\n## Current user state\n\n";
  ...
  return (function* () {
    try {
      ...
      for (const row of reader.prepare("SELECT collected_at AS collectedAt,payload FROM collection_snapshots ORDER BY collected_at,id").iterate()) {
        ...
        for (const item of parseSnapshot(row).map(record)) yield `${item.rank}. ${item.title}\n   ${item.excerpt}\n   ${item.heat}, ${item.answers} answers, ${states.get(item.url) ? "read" : "unread"}\n   ${item.url}\n`;
      }
      yield "\nCurrent user state:\n";
...
export function createExport(filename, { pluginId = "zhihu-hot", pluginVersion, format = "json", exportedAt = new Date().toISOString() } = {}) {
