...
document.body.innerHTML = `
  <div id="workspace" class="workspace">
    <header class="workspace-header">
      <div class="header-inner"><div class="brand-lockup"><span class="source-mark" aria-hidden="true">掘</span><div class="brand-copy"><strong>掘金</strong><span id="refresh-time">正在读取保留内容...</span></div></div>
      <div class="header-actions"><button id="refresh" class="icon-button" type="button" aria-label="刷新掘金" title="刷新"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 11a8.1 8.1 0 0 0-15.5-2M4 4v5h5M4 13a8.1 8.1 0 0 0 15.5 2M20 20v-5h-5"/></svg></button><button id="settings" class="icon-button" type="button" aria-label="打开刷新设置" title="刷新设置"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.7 1.7 0 0 0 .34 1.88l-2.83 2.83A1.7 1.7 0 0 0 14 20.9h-4A1.7 1.7 0 0 0 9 19.36l-2.83 2.83A1.7 1.7 0 0 0 4.63 15 1.7 1.7 0 0 0 3.08 14v-4A1.7 1.7 0 0 0 4.64 9a1.7 1.7 0 0 0-.34-1.88l2.83-2.83A1.7 1.7 0 0 0 9 4.63 1.7 1.7 0 0 0 10 3.08h4A1.7 1.7 0 0 0 15 4.64l2.83-2.83A1.7 1.7 0 0 0 19.37 9 1.7 1.7 0 0 0 20.92 10v4A1.7 1.7 0 0 0 19.4 15Z"/></svg></button></div></div>
    </header>
    ...
    <div id="warning" class="warning" role="alert" hidden><span><strong>刷新失败。</strong> 当前继续显示上次成功保留的文章。</span><button id="retry" class="text-button" type="button">重试</button></div>
    <main class="workspace-main">



      <section class="section-head"><h2>热门文章</h2><p class="section-count"><strong id="article-count">—</strong> 篇保留</p></section>
      <ol id="article-list" class="article-list" aria-live="polite"><li class="loading">正在读取保留内容...</li></ol>
      <section id="empty" class="empty" hidden><span class="state-mark" aria-hidden="true">掘</span><h2 id="empty-title">还没有保留内容</h2><p id="empty-copy">首次刷新后，文章会保存在此设备上。</p><button id="empty-refresh" class="button button--primary" type="button">立即刷新</button></section>
    </main>
  </div>
  <div id="scrim" class="scrim" hidden></div>
  <aside id="settings-sheet" class="sheet" role="dialog" aria-modal="true" aria-labelledby="settings-title" hidden><header class="sheet-head"><div><p>工作区设置</p><h2 id="settings-title">刷新与内容设置</h2></div><button id="close-settings" class="icon-button" type="button" aria-label="关闭设置">×</button></header><form id="settings-form"><fieldset><legend>刷新策略</legend><label><input type="radio" name="policy" value="manual"><span>仅手动</span></label><label><input type="radio" name="policy" value="disabled"><span>停用刷新</span></label><label><input type="radio" name="policy" value="fixed"><span>固定间隔</span></label></fieldset><label class="select-field" for="interval">刷新间隔</label><select id="interval" name="intervalMinutes"><option value="15">每 15 分钟</option><option value="30">每 30 分钟</option><option value="60">每小时</option><option value="360">每 6 小时</option><option value="720">每 12 小时</option><option value="1440">每天</option></select><label class="select-field" for="category">内容分类</label><select id="category" name="category"><option value="backend">后端</option><option value="frontend">前端</option><option value="android">Android</option><option value="ios">iOS</option><option value="ai">人工智能</option></select><label class="select-field" for="limit">文章数量</label><select id="limit" name="limit"><option value="10">10 篇</option><option value="20">20 篇</option><option value="30">30 篇</option><option value="40">40 篇</option><option value="50">50 篇</option></select><label class="select-field" for="retention">历史保留</label><select id="retention" name="retentionDays"><option value="7">7 天</option><option value="30">30 天</option><option value="90">90 天</option></select><div class="sheet-actions"><button id="cancel-settings" class="button" type="button">取消</button><button class="button button--primary" type="submit">保存</button></div></form></aside>
  <aside id="detail-sheet" class="detail-sheet" role="dialog" aria-modal="true" aria-labelledby="detail-title" hidden><header class="detail-head"><div><p id="detail-context">掘金</p><h2 id="detail-title">文章</h2></div><button id="close-detail" class="icon-button" type="button" aria-label="关闭详情">×</button></header><div class="detail-body"><div id="detail-meta" class="detail-meta"></div><p id="detail-brief" class="detail-description"></p><div class="detail-actions"><button id="detail-open" class="button button--primary" type="button">打开掘金文章</button></div></div></aside>
`;
